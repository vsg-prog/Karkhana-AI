/**
 * Realtime Nitya — microphone & speaker device picker (card rt-8, Phase 1).
 *
 * Lets the user choose WHICH microphone the voice loop captures and WHICH speaker
 * it plays Nitya's voice through. Selections are held in the realtime session
...
import { useRealtimeNitya } from './session';
...
export function RealtimeDevicePicker(): React.ReactElement {
  const { deviceId, setDeviceId, outputDeviceId, setOutputDeviceId } = useRealtimeNitya();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: 280 }}>
      ...
      {!labelled && (
        <span style={{ fontSize: 12, lineHeight: '16px', color: 'var(--cth-ink-500)' }}>
          ...
          The microphone choice applies the next time Nitya connects; the speaker switches live.
        </span>
      )}
    </div>
  );
