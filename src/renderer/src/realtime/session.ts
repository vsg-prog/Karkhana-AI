/**
 * Realtime Nitya — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * get_config / get_memory / get_activity) and god's rt-6 "Nitya" persona, so the
 ...
 * loop at a time) exposed through a `useRealtimeNitya()` hook via useSyncExternalStore.
 *
...
export interface RealtimeNityaState {
...
/** Warm openers Nitya leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Nitya here — what can I do for you?",
...
/** Nitya's voice persona (rt-6 — the final Phase-1 instructions, authored by god). Nitya
 *  is READ-ONLY: he reports on the hive via the rt-4 read-tools but takes no actions yet. */
const NITYA_PERSONA =
  `You are Nitya — the voice of the orchestrator ("god") of a hive of autonomous Claude coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
/** The <audio> sink for Nitya's voice. */
...
function setState(patch: Partial<RealtimeNityaState>): void {
  state = { ...state, ...patch };
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Nitya';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Nitya's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Nitya's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Nitya
    ...
    // Floor deltas — silent appends that keep Nitya's picture live without
    ...
    // subscribe so a detected completion makes Nitya speak it unprompted.
    ...
    // Open the conversation: have Nitya speak a warm greeting as his first turn
...
function getSnapshot(): RealtimeNityaState {
  return state;
}

/**
 * React binding for the Realtime Nitya voice loop. Returns the current state plus
 ...
 */
export function useRealtimeNitya(): RealtimeNityaState & {
