...
/** Nitya's control surface. Shown instead of the plain terminal/files panel
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 1, minWidth: 0 }}>
            ...
            <span style={{
              ...
              whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'
            }}>{getLocalizedCharacterName('nitya', selectedLanguage)} runs the floor</span>
          </div>
        </div>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {tab === 'terminal' && (
          isFullscreenedHere ? (
            ...
            <Centered>Nitya has no live terminal.</Centered>
          )
        )}
...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  // The harness's own default model (Settings → default model). Nitya and every
  ...
  const [dispatchTo, setDispatchTo] = useState<string>(''); // '' = Nitya decides
  ...
  const restartWithModel = async (
    ...
    try {
      ...
      const hive = a.isGod
        ...
        : a.isAssistant
        ? { id: a.id, name: a.name, cwd: a.cwd, provider, isAssistant: true, role: "Nitya's prep assistant" }
  ...
  const dispatch = async () => {
    ...
    setDispatchMsg(res.ok
      ? `sent to Nitya${suggested ? ` (suggesting ${suggested.name})` : ''}`
      : `failed: ${res.error ?? '?'}`);
  ...
  const assignIssue = (issue: GHIssue) => {
    ...
    setDispatchTo(''); // Nitya decomposes and assigns — no more broadcast blasts
  };
  ...
  return (
    <Scroll>
      <Section title="DISPATCH — VIA NITYA">
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
          ...
          <Select value={dispatchTo} onChange={setDispatchTo}>
            <option value="">Nitya decides</option>
        ...
        <textarea
          ...
          rows={2}
          placeholder="Describe the task… (Nitya decomposes, writes the card, and assigns)"
          style={textareaStyle}
        />
                ...
                <PixelButton
                  ...
                  onClick={async () => {
                    ...
                    if (engineProvider !== currentProvider) {
                      if (!window.confirm("This restarts Nitya; a conversation on a different engine can't be resumed.")) return;
                    }
