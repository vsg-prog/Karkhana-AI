...
export interface Agent {
  ...
  lastPrompt?: string;
  /** the orchestrator ("god") agent — seated in Nitya's room, runs the floor */
  isGod?: boolean;
  /** Nitya's prep assistant — send-only; enriches prompts and forwards them to
...
/** Lifecycle of the god agent ("Nitya") bootstrap on launch.
 ...
 *  don't see the "add agent" prompt before Nitya has clocked in. */
...
interface State {
   ...
   *  secret broker, never the store). Gates the Realtime Nitya voice toggle the
...
export const useStore = create<State>((set) => ({
  ...
  addAgent: (agent) =>
    set((s) => {
      ...
      // GOD enters at the HEAD, everyone else at the tail. Nitya's position was
