...
export interface HiveAgentMeta {
  ...
  cwd: string;
  isGod?: boolean;
  /** Nitya's prep assistant — send-only; enriches prompts and forwards them. */
  isAssistant?: boolean;
}
...
export interface HarnessConfig {
  ...
  /** Realtime Nitya voice loop — true ONLY while a session holds the mic
...
const api = {
   ...
   *  {} for the whole floor. Backs Realtime Nitya's get_messages. The renderer
   ...
   *  archived agents. Backs Realtime Nitya's get_agent_detail / list_agents. */
  ...
  // ─── Slack integration (Slack message → Nitya's queue) ─────────────────────
  ...
  // Realtime Nitya (voice orchestrator) — MAIN mints a short-lived EPHEMERAL token
  ...
  // the tiering, two-step verbal confirm, hard allowlist, and nitya-voice
  ...
  // human-speakable line Nitya relays; the rest is context for a toast/log.
